...
import DropdownMenuPost from '@/components/Radix-Burger';
import DropdownMenuComment from '@/components/Radix-Burger-comment';
...
export default async function editProfile() {
  ...
  const snowboardingPosts = (
    await db.query(
      `SELECT * FROM snowboarding_posts WHERE userid = $1 ORDER BY id DESC `,
      [user.username]
    )
  ).rows;
  ...
  return (
    <div className='masterProfileContianer'>
      <h1> Welcome {user.username} to your Profile</h1>
      <h2> Here is your bio feel free to update it.</h2>
